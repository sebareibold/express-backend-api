...
class CartsManager {
  ...
  async getCart(id) {
    try {
      ...
      // Usar populate para obtener información completa de los productos
  ...
  async updateCartWithNewProductList(cartId, newProducts) {
    try {
      ...
      // Guardar los cambios
      const updatedCart = await cart.save()
      console.log(`Carrito ${cartId} actualizado con nueva lista de productos.`)

      return updatedCart
    } catch (error) {
      console.error("Error al actualizar carrito con nueva lista:", error.message)
      return null
    }
  }
